...
{
public class MiniPostProcessRenderPass : ScriptableRenderPass
{
	//构造方法中接收这个Render Pass的Profiler标识与后处理组件列表，以每个组件的名称作为它们渲染时的Profiler标识。

	List<MiniVolumeComponent> volumeComponents; // 所有自定义后处理组件
	...
	public MiniPostProcessRenderPass(string profilerTag, List<MiniVolumeComponent> volumeComponents)
	{
